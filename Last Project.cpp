...
//Digunakan untuk melakukan penginputan nama player
typedef struct{
	char name[200];
	int damage;
	int maxHealth;
...
//Digunakan untuk melakukan penginputan nama monster
typedef struct{
	char name[200];
	int damage;
	int maxHealth;
...
//Untuk data monsters
int isNew = false; //Digunakan untuk mengecek apakah nama karakternya ada atau tidak
monster monsters[] = {
					{
						"Goblin",
						5,
...
//Untuk melakukan perbandingan dengan inputan
classes loadClass(char c[]){
	...
	else if(strcmp(c, "Archer") == 0){ //Archer
...
void loadAccount(){
	...
	char tempclass[200];//mengecek inputan dari user
...
void heal(){
	...
	if(playerCharacter.health == playerCharacter._class.maxHealth){
			...
			int input;
			do{
				printf("Do you want to heal yourself for %d gold ?\n", healPrice);
				printf("1. Yes\n2. No\n>> ");
				scanf("%d", &input); getchar();
		...
		switch(input){
			case 1:
				...
				if(playerCharacter.health <= 0){ //Player lose
					...
					playerCharacter.exp = 0;
					playerCharacter.health = playerCharacter._class.maxHealth;
					printf("You lose!\nYou lost all your golds and exp\nPress any key to continue...");
					getchar();
					return;
				}
			break;
...
int main(){
	...
	//apabila pilih 4 maka programnya akan berhenti
